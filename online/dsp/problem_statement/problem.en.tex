...
The software that defines the DSP's behavior consists of a sequence of $N$ (up to 256) machine instructions (also called assembly instructions), numbered from $0$ to $N-1$. There are 7 types of instructions: \texttt{CONST}, \texttt{ADD}, \texttt{SUB}, \texttt{JNZ}, \texttt{INPUT}, \texttt{OUTPUT}, and \texttt{HALT}. Each instruction has 0--2 parameters, each of which is an integer $0..255$. The DSP also has a working memory consisting of 256 registers, each containing a byte (i.e., an integer $0..255$).
...
The first line contains an integer $N$, $0<N<256$, the number of machine instructions in the program. Then follow $N$ lines with instructions. Each instruction consists of the instruction name, followed by the instruction's parameters. Each parameter is described by an integer in the range $0..255$. After the $N$ lines of machine instructions, a number of integers in the range $0..255$ follow, one per line. These constitute the input to the DSP, i.e., the series of numbers that the DSP program's \texttt{INPUT} instructions read from.
...
\item The number of instructions that need to be executed will not exceed one million.
