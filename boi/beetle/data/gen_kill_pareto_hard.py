...
# To allow a tiny bit of variation across seeds so Kattis doesn't just cache perfectly,
# we can perturb some middle drops safely without destroying the structure.
mutatable = [i for i, v in enumerate(drops) if -5000 < v < 5000 and v != 0]

for _ in range(5):
    idx = random.choice(mutatable)
    candidate = drops[idx] + random.choice([-1, 1])
    if candidate not in drops and candidate != 0:
        drops[idx] = candidate
